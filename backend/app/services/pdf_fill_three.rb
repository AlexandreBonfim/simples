...
class PdfFillThree
  def initialize(worker)
    @worker = worker
    @template_path = Rails.root.join("app/assets/template3.pdf")
    @output_path = Rails.root.join("tmp", "acta_test_3.pdf")
  end
  ...
  def call
    ...
    canvas.text(@worker[:full_name], at: [130, 650])       # El trabajador
    canvas.text(@worker[:doc_number], at: [105, 620])       # con D.N.I.
    ...
    canvas.text(day, at: [320, 230])    # Estimated position for day (e.g., "02")
    canvas.text(month, at: [375, 230])  # Estimated position for month (e.g., "julio")
